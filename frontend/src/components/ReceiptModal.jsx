...
const ReceiptModal = ({ isOpen, onClose, orderData }) => {
  ...
  const downloadPDF = () => {
    ...
    const opt = {
      margin:       [10, 10],
      filename:     `ticket-${orderData.paypal_id || orderData.id}.pdf`,
        ...
        <div className="receipt-content" ref={receiptRef}>
          ...
          <div className="receipt-details">
            <div className="receipt-row">
              <span>Fecha</span>
              <strong>{orderData.created_at ? new Date(orderData.created_at).toLocaleDateString() : new Date().toLocaleDateString()}</strong>
            </div>
            <div className="receipt-row">
              <span>ID Transacción</span>
              <strong>{orderData.paypal_id || orderData.id}</strong>
            </div>
            <div className="receipt-row">
              <span>Estado</span>
          ...
          <div className="receipt-items">
            ...
            {orderData.detalles.map((item, index) => (
              <div key={index} className="receipt-item-row">
                ...
                <span className="item-price">{formatCurrency(item.precio_unitario * item.cantidad)}</span>
              </div>
            ))}
          </div>
